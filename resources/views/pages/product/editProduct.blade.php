...
@section('content')
    <form method="post" action="">
        ...
        <div class="form-floating mt-3">
            <input type="text" class="form-control @error('name') is-invalid @enderror" value="{{$product->name}}" name="name" id="floatingName" placeholder="Имя" required>
        ...
        <div class="form-floating mt-3">
            <select class="form-select" id="floatingSelect" name="measurement" aria-label="Floating label select example">
                ...
                <option value="1" @if($product->measurement === '1') selected @endif>шт.</option>
                <option value="2" @if($product->measurement === '2') selected @endif>мес.</option>
                <option value="3" @if($product->measurement === '3') selected @endif>др.</option>
            </select>
        ...
        <div class="form-floating mt-3">
            <select class="form-select" id="floatingSelect" name="category_id" aria-label="Floating label select example">
                ...
                @foreach($categories as $category)
                    <option value="{{$category->name}}" @if($category->id === $product->category_id) selected @endif>{{$category->name}}</option>
                @endforeach
            </select>
